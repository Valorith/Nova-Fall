...
<template>
  <div class="selector-node" :class="{ selected: props.selected }">
    <Handle type="target" :position="Position.Top" class="handle handle-target" />

    <div class="node-header">
      <span class="node-icon">?</span>
      <span class="node-title">{{ data.label || 'Selector' }}</span>
    </div>

    <div class="node-body">
      <span class="node-description">Try until success</span>
...
.selector-node.selected {
  border-color: #fbbf24;
  box-shadow: 0 0 0 2px rgba(251, 191, 36, 0.5), 0 4px 12px rgba(245, 158, 11, 0.4);
}
