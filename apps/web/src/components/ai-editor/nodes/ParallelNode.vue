...
const policy = computed(() => {
  return (props.data.params?.policy as string) || 'all';
});
...
<template>
  ...
  >
    ...
    <div class="node-header">
      ...
      <span class="node-title">{{ data.label || 'Parallel' }}</span>
    </div>
    ...
    <div class="node-body">
      ...
      <template v-else>
        <span class="node-description">Run all ({{ policy }})</span>
...
.parallel-node.selected {
  border-color: #c084fc;
  box-shadow: 0 0 0 2px rgba(192, 132, 252, 0.5), 0 4px 12px rgba(168, 85, 247, 0.4);
}
...
.parallel-node.collapsed {
  border-style: dashed;
}

.parallel-node.collapsed .node-body {
